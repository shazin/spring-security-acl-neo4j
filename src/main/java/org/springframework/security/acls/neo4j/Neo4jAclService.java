...
public class Neo4jAclService implements AclService {
	...
	public Neo4jAclService(GraphDatabaseService graphDatabaseService, LookupStrategy lookupStrategy, AclCache aclCache) {
		Assert.notNull(aclCache, "AclCache can not be null");
		Assert.notNull(lookupStrategy, "LookStrategy can not be null");
		Assert.notNull(graphDatabaseService, "GraphDatabaseService can not be null");
	...
	public List<ObjectIdentity> findChildren(ObjectIdentity parentIdentity) {
		...
		Result<Map<String, Object>> result = neo4jTemplate.query(findChildrenSql, params);
		Iterator<Map<String, Object>> it = result.iterator();
		Map<String, Object> data = null;
		while(it.hasNext()) {
			data = it.next();
			objects.add(new ObjectIdentityImpl((String) data.get("className"), (Long) data.get("aclId")));
		}
